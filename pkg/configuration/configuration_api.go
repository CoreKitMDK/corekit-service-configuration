...
func (c *API) Get(key string) (string, error) {
	req, err := http.NewRequest("GET", c.Url+"?key="+key, nil)
	if err != nil {
		return "", err
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		return "", fmt.Errorf("API request failed with status: %d from repsponder: %s with log: %s", resp.StatusCode, responder, responderLog)
	}
